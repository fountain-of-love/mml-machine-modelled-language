...
The Representational Leverage Proposition is the architectural mechanism within MML's first hypothesis, Semantic Representation: making task-relevant meaning explicit, addressable, and governed may enable fixed ordinary mathematics to produce more useful and attributable semantic behavior.
...
The systematic test is defined by [Experiment 1 — Represent the Meaning. Change the Field.](benchmark/experiment-1-semantic-representation.md). The current human-readable development evidence is reported in [Semantic Representation Benchmark v1](benchmark/results/semantic-representation-v1.md).
