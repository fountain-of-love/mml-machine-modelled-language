...
The Compiled Knowledge Reuse Proposition is the architectural mechanism within MML's second hypothesis, Knowledge State Execution: established knowledge compiled once into governed executable state may serve repeated queries without reconstructing equivalent task state from its source at every use.
...
The systematic test is defined by [Experiment 2 — Compile Once. Execute What Is Known.](benchmark/experiment-2-knowledge-state-execution.md). The current human-readable development evidence is reported in [Knowledge State Execution Experiment v1](benchmark/results/knowledge-state-v1.md).
