...
Explicit governed stage-local scopes can resolve successive semantic regions while preserving antecedent provenance and releasing coordinates that are not properties of the later region.
...
The result is **consistent** with the bounded stage-reset claim. All 6 authored stage-local compositions resolved their predeclared semantic regions, all antecedent regions remained visible in the trace, and deterministic replay matched. All 3 flat cumulative controls remained unresolved because their complete coordinate unions had no governed common target.
...
The unchanged synthetic legal state contains 33 concepts, 44 dimensions, and 150 ordinary single-trait relations. Its compiled identity is `sha256:5c0eea4aaf7950e39f646bfb8ead25c32a3329d17e6ca263dd9629672a78c5ab`. The state does not contain typed cross-level transition edges or bespoke nodes for the tested stage combinations.
...
The fixture lifecycle is `AUTHORED_DEVELOPMENT` and the probe holdout status is `not_held_out`. Its epistemic classification is `synthetic_doctrinal_construction`. That classification is operational metadata and cannot become an established fact, inference, duty, or violation through activation.
...
The experiment compiled the legal state once, then executed 3 two-stage transitions (6 stages), 3 flat cumulative controls, and exact deterministic repeats using policy `cross-level-transition-development-v1`.
...
| Measure | Result |
| --- | ---: |
| Stage-local resolution | 100.0% |
| Declared-region match | 100.0% |
| Flat-control non-resolution | 100.0% |

### Semantic transitions and scope changes

| Probe | Antecedent region | Consequent region | Retained | Introduced | Released |
| --- | --- | --- | --- | --- | --- |
| `cross_access_to_evidence` | `selective_access` | `evidence_asymmetry` | exclusive_control, incomplete_disclosure | evidence, dispute | access, source_coverage |
| `cross_profile_to_contestability` | `opaque_profiling` | `uncontestable_adverse_decision` | profiling, decision_logging, contestability | incomplete_disclosure, dispute | personal_data |
| `cross_record_to_evidence` | `recording_transparency_issue` | `evidential_record` | recording | professional_record, reconstruction, evidence | personal_data, transparency, metadata |
...
| --- | ---: | --- | --- | ---: | ---: | ---: |
| `cross_access_to_evidence` | 1 | `selective_access` | `selective_access` | 1 | 0.000000 | 1.000000 |
| `cross_access_to_evidence` | 2 | `evidence_asymmetry` | `evidence_asymmetry` | 1 | 0.196663 | 0.104927 |
| `cross_profile_to_contestability` | 1 | `opaque_profiling` | `opaque_profiling` | 1 | 0.000000 | 1.000000 |
| `cross_profile_to_contestability` | 2 | `uncontestable_adverse_decision` | `uncontestable_adverse_decision` | 1 | 0.000000 | 1.000000 |
| `cross_record_to_evidence` | 1 | `recording_transparency_issue` | `recording_transparency_issue` | 1 | 0.000000 | 1.000000 |
| `cross_record_to_evidence` | 2 | `evidential_record` | `evidential_record` | 1 | 0.000000 | 1.000000 |
...
| --- | --- | --- | --- | --- | ---: |
| `cross_access_to_evidence` | `UNRESOLVED` | `UNSUPPORTED_COMBINATION` | `UNSUPPORTED_COMBINATION` | `selective_access` | 0 |
| `cross_profile_to_contestability` | `UNRESOLVED` | `UNSUPPORTED_COMBINATION` | `UNSUPPORTED_COMBINATION` | `uncontestable_adverse_decision` | 0 |
| `cross_record_to_evidence` | `UNRESOLVED` | `AMBIGUOUS_FIELD` | `UNSUPPORTED_COMBINATION` | `recording_transparency_issue` | 0 |
...
**Conformity judgment: `CONSISTENT`.** Every declared criterion passed within this authored fixture. Stage-local composition resolved 6/6 declared regions, all three flat conjunction controls remained unresolved, antecedent provenance and scope changes were complete, and replay was deterministic.
...
| Criterion | Result |
| --- | --- |
| `all_stage_local_regions_resolve` | pass |
| `all_regions_match_predeclared_targets` | pass |
| `antecedent_regions_are_preserved_in_trace` | pass |
| `level_changes_explicitly_release_constraints` | pass |
| `flat_conjunction_control_remains_unresolved` | pass |
| `repeat_execution_is_deterministic` | pass |
| `epistemic_classification_is_not_promoted` | pass |
...
The stage-reset treatment was authored after the cumulative-intersection failure. A CONSISTENT result establishes only reproducible behavior of this explicit scoped operation in the co-authored synthetic fixture.

Authored stage-reset transition over one synthetic legal state; not automatic scope inference, typed relation traversal, legal causation, factual adjudication, or generalization.

This report follows the [OSCARC methodology](../oscarc-methodology.md). The [machine-readable JSON artifact](../../../benchmark/results/cross-level-semantic-transition-v1.json) remains authoritative for measurements, complete transition traces, conformity inputs, artifact identities, and provenance.
