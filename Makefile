...
test:
	...
	PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m experiments.knowledge_state_execution.semantic_operator_benchmark --check >/dev/null
...
semantic-operator-benchmark:
	PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m experiments.knowledge_state_execution.semantic_operator_benchmark --write

semantic-operator-benchmark-check:
	PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m experiments.knowledge_state_execution.semantic_operator_benchmark --check >/dev/null
...
experiment-4-1-benchmark:
	PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m experiments.semantic_navigation.compiled_encyclopedic_navigation_benchmark --write
